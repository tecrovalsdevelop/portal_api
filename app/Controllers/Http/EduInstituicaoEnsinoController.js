...
const Instituicao = use('App/Models/EduInstituicaoEnsino')
...
class EduInstituicaoEnsinoController {
    /**
    ...
    async intituicaoBypaisnivel({ params, request, response }) {
        try {
            ...
            //console.log(params)
            let instituicao = await Instituicao.query()
                .where({
    ...
    async intituicaoBypaisprovincianivel({ params, request, response }) {
        try {
            ...
            let result = await Instituicao.query()
    ...
    /*   async store({ request, response }) {
           try {
               const data = request.only(requestFields2);
               const instituicao = await Instituicao.find(data.id); 
               //console.log(instituicao)
               if (instituicao) {
                 ...
                 instituicao = await Instituicao.create(data);
   
               return ResponseHelper.getSuccessResponse("Instituição criada com sucesso", instituicao)
           } catch (err) {
               //console.log(err.message)
    ...
    async save({ request, response }) {
        try {
            ...
            var instituicao
            if (data.id) {
                instituicao = await Instituicao.find(data.id);
                instituicao.merge(data);
                await instituicao.save();
            } else {
                instituicao = await Instituicao.create(data);
            }
            return ResponseHelper.getSuccessResponse("Instituição cadastrado com sucesso", instituicao)

        } catch (err) {
            //console.log(err.message)
            return ResponseHelper.getErrorResponse("Erro ao cadastrado a Instituição")
        }
    }
    ...
    async store({ request, response }) {
        try {
            ...
            const data = request.only(requestFields);
            const edu_instituicao = await Instituicao.create(data);
            return ResponseHelper.getSuccessResponse("Curso criado com sucesso", edu_instituicao)
...
module.exports = EduInstituicaoEnsinoController